# Handling Large Payloads with Binary Cache

## Introduction

The Binary Cache is a temporary workspace provided by the MultiChain
node. It allows R users to bypass memory limits by streaming data into
the node in small pieces, publishing it as a single unit, and retrieving
...
When dealing with large data, you create a cache item and append data to
...
Once the data is assembled in the cache, we can publish it to a stream.
By using the `offchain` option, the data remains in the node’s local
...
When you need to read a large item published by another node, you can
copy it directly from the transaction output into a *new* binary cache
item.
...
Finally, we can extract the hex data from the cache (or a specific part
...
1.  **Initialize Binary Cache**: Creating a new temporary data item with
    `mc_create_binary_cache`.
2.  **Append Data**: Uploading data in chunks with
    `mc_append_binary_cache`.
3.  **Publish Off-chain**: Recording only a hash on the blockchain while
    keeping the large payload locally with `mc_publish`.
4.  **Retrieve to Cache**: Moving data from a transaction output
    directly back into a cache item with `mc_txout_to_binary_cache`.
5.  **Examine and Cleanup**: Reading the final data and deleting the
    temporary cache items with `mc_get_tx_out_data` and
