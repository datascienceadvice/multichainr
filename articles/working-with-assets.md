# Managing Assets and Tokens

## Introduction

The `multichainr` package provides a high-level R interface to the
MultiChain JSON-RPC API. This vignette demonstrates the lifecycle of
managing both **fungible assets** (like currencies or loyalty points)
...
MultiChain operates as a permissioned blockchain. We start by
...
By default, MultiChain is secure. Addresses must be granted specific
...
Fungible assets are divisible and interchangeable. We can create an
...
In MultiChain 2.x, NFTs are managed as unique tokens within a
non-fungible “Parent Asset.”
...
Once the parent exists, we can issue individual tokens (NFTs). Each
...
When the work is complete, it is important to stop the node and, if
...
4.  Issue and query unique tokens (NFTs) via `mc_issue_token` and
    `mc_get_token_info`.
