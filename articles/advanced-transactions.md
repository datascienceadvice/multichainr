...
In a MultiChain network, atomic swaps are performed using a sequence of
“raw exchange” commands. This process involves locking a transaction
output (UTXO), creating a partial transaction offer, and having a
...
To demonstrate a swap, we need two participants and two different
assets.
...
Trader A wants to exchange **10 units of AssetA** for **5 units of
AssetB**. First, Trader A must “lock” their AssetA to prepare it for the
exchange.
...
Trader B receives the `offer_hex`, inspects it, and decides to accept it
...
The exchange is now a fully formed raw transaction. It must be broadcast
...
1.  **Prepare Locked Outputs**: Using `mc_prepare_lock_unspent_from` to
    set aside specific assets for trade.
2.  **Create Offers**: Generating a partial transaction hex with
    `mc_create_raw_exchange`.
3.  **Audit Offers**: Using `mc_decode_raw_exchange` to verify the
    contents of a trade before signing.
4.  **Complete and Finalize**: Combining multiple parties’ inputs into a
    single transaction with `mc_complete_raw_exchange`.
5.  **Execution**: Broadcasting raw hex to the blockchain using
